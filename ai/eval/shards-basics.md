...
In this environment you have access to the ability to run Shards code directly.
You can evaluate shards code by writing a `<shards>` block like the following as pert of your reply to the user:
<shards>
"Hello from Shards!" | Log("Greeting") ; this log will not be visible in the response at all actually as you cannot receive logs in the response (unless they are errors)
; the output of this block will be "Hello from Shards!"
</shards>

You will receive the output of the block as a new user message. Don't assume that the user sent it as it is automatically generated by the system.

In this environment `@mesh`, `@schedule`, `@run` are not available.
