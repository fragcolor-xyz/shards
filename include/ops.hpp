...
ALWAYS_INLINE inline bool operator==(const CBVar &a, const CBVar &b) {
  ...
  case Float2: {
    ...
    return true;
#else
    return diff[0] <= vepsi[0] && diff[1] <= vepsi[0];
#endif
  }
  case Float3: {
...
ALWAYS_INLINE inline bool operator<(const CBVar &a, const CBVar &b) {
  ...
  case Float2: {
    ...
    return false;
#else
    return a.payload.float2Value[0] < b.payload.float2Value[0] &&
           a.payload.float2Value[1] < b.payload.float2Value[1];
  }
#endif
  case Float3: {
  ...
  ALWAYS_INLINE inline bool operator<=(const CBVar &a, const CBVar &b) {
...
#ifndef __EMSCRIPTEN__
      CBInt2 vec =
          a.payload.float2Value <= b.payload.float2Value; // cast to int
      for (auto i = 0; i < 2; i++)
        if (vec[i] == 0)
          return false;
      return true;
    ...
    case Float3: {
      CBInt3 vec =
          a.payload.float3Value <= b.payload.float3Value; // cast to int
      for (auto i = 0; i < 3; i++)
        if (vec[i] == 0)
          return false;
      return true;
    ...
    case Float4: {
      CBInt4 vec =
          a.payload.float4Value <= b.payload.float4Value; // cast to int
      for (auto i = 0; i < 4; i++)
        if (vec[i] == 0)
          return false;
      return true;
          ...
          for (uint32_t j = 0; j < btypes; j++) {
            if (a.table.types.elements[i] == b.table.types.elements[j]) {
              if (a.table.keys.elements) { // this is enough to know they exist
                if (strcmp(a.table.keys.elements[i],
                           b.table.keys.elements[j]) == 0) {
                  goto matched_table;
                }
              } else {
                goto matched_table;
    ...
    inline bool operator!=(const CBExposedTypeInfo &a,
                           const CBExposedTypeInfo &b);
  ...
  res = res ^                                                                  \
    ...
    template <> struct hash<CBVar> {
      std::size_t operator()(const CBVar &var) const {
        ...
        using std::size_t;
        using std::string;
        const static CBCore core =
            chainblocksInterface(CHAINBLOCKS_CURRENT_ABI);
