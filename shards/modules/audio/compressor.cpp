...
struct Compressor {
  static SHOptionalString help() {
    return SHCCSTR("This shard applies dynamic range compression to audio data. It reduces the volume of loud sounds or amplifies "
  ...
  static SHOptionalString outputHelp() {
    return SHCCSTR("Outputs the compressed audio data as an Audio chunk.");
  }
  ...
  PARAM_IMPL(PARAM_IMPL_FOR(_threshold), PARAM_IMPL_FOR(_ratio), PARAM_IMPL_FOR(_attack), 
             PARAM_IMPL_FOR(_release), PARAM_IMPL_FOR(_makeupGain));
  ...
  inline float timeToCoeff(float timeMs, float sampleRate) {
...
void registerCompressorShards() {
  REGISTER_SHARD("Audio.Compressor", Compressor);
}
} // namespace Audio
} // namespace shards