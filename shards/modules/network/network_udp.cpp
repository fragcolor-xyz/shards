...
struct NetworkPeer {
  ...
  void sendVar(const SHVar &input) {
    ...
    if (size > MAX_PKT_SIZE) {
      // send in chunks
      size_t chunks = size / MAX_PKT_SIZE;
      size_t remaining = size % MAX_PKT_SIZE;
      auto ptr = _sendWriter.data();
      for (size_t i = 0; i < chunks; ++i) {
        auto err = ikcp_send(kcp, ptr, MAX_PKT_SIZE);
        if (err < 0) {
      ...
      for (auto &[end, peer] : _end2Peer) {
        ...
        if (peer->tryReceive(context)) {
          ...
          try {
            ...
            if (peer_->recvBuffer.size() > MAX_PKT_SIZE) {
...
struct Client : public NetworkBase {
  ...
  SHVar activate(SHContext *context, const SHVar &input) {
    ...
    if (_peer.tryReceive(context)) {
      ...
      if (_peer.recvBuffer.size() > MAX_PKT_SIZE) {
        // do this async as it's a big buffer
        await(
            context,
            [&]() {
