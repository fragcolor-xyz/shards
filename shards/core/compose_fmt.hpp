...
template <> struct formatter<shards::compose::VariableAccessor> {
  ...
  template <typename FormatContext> auto format(shards::compose::VariableAccessor const &va, FormatContext &ctx) const {
...
template <> struct formatter<shards::compose::VariableAccessorChain> {
  template <typename ParseContext> constexpr auto parse(ParseContext &ctx) { return ctx.begin(); }
  template <typename FormatContext> auto format(shards::compose::VariableAccessorChain const &chain, FormatContext &ctx) const {
...
template <> struct formatter<shards::compose::VersionedReference> {
  template <typename ParseContext> constexpr auto parse(ParseContext &ctx) { return ctx.begin(); }
  template <typename FormatContext> auto format(shards::compose::VersionedReference const &ref, FormatContext &ctx) const {
