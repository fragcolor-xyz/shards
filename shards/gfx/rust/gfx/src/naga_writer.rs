...
pub mod native {
  ...
  fn convert_string(ptr: *const c_char) -> Option<String> {
    if !ptr.is_null() {
  ...
  impl<T, U> From<Handle<U>> for naga::Handle<T> {
    fn from(h: Handle<U>) -> naga::Handle<T> {
      unsafe { naga::Handle::<T>::new(NonZeroU32::new_unchecked(h.index)) }
    }
  }
...
#[no_mangle]
pub unsafe extern "C" fn nagaFunctionSetBody(
  ctx_: *mut NagaFunctionWriteContext,
  stmt: native::Block,
) {
...
#[repr(C)]
pub struct NagaFunctionWriteContext {
  outer: *mut NagaWriter,
  fun: naga::Function,
}

#[repr(C)]
pub struct NagaFunctionDescriptor {
  name: *const c_char,
}

pub type FunctionWriteCallback =
  extern "C" fn(ctx: *mut NagaFunctionWriteContext, user_data: *mut c_void);

#[no_mangle]
pub unsafe extern "C" fn nagaFunctionStoreExpression(
  writer_: *mut NagaFunctionWriteContext,
...
#[no_mangle]
pub unsafe extern "C" fn nagaFunctionStoreConstant(
  writer_: *mut NagaFunctionWriteContext,
  c: native::Constant,
  ...
  let outer = writer.outer.as_mut().expect("Invalid writer");
  outer
    .module
    .constants
...
pub unsafe extern "C" fn nagaAddFunction(
  ...
  desc_: *const NagaFunctionDescriptor,
  cb: FunctionWriteCallback,
  user_data: *mut c_void,
) {
  let writer = writer_.as_mut().expect("Invalid writer");
  let desc: &NagaFunctionDescriptor = desc_.as_ref().expect("Invalid descriptor");
  //   let module = naga::Module::default();
  // module.constants.append(Constant(, span)

  let mut ctx = NagaFunctionWriteContext {
    outer: writer_,
    fun: naga::Function {
  ...
  //   };
  // ep.
  // module.entry_points.push(value)
  // writer.module = Some();
