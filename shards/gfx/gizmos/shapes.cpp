#include "shapes.hpp"
#include "linalg.h"
#include <gfx/text/text_placer.hpp>
...
void ShapeRenderer::addText(float3 origin, float3 xBase, float3 yBase, float size, std::string_view text, float4 color,
                            float2 align) {
  static auto defaultFont = text::FontMap::getDefault();
  text::TextPlacer placer;

  const uint32_t fontSize = 8;
  float actualTextScale = size / float(fontSize);
  const auto &fontSizeObj = defaultFont->getFontSize(fontSize);
  placer.verticalAlignOrigin(fontSizeObj, align.y);
  placer.appendString(fontSizeObj, text);

  textMesh.appendText( //
      placer,          //
      text::MeshBuffer::TextParams{
          .offset = origin,
          .right = xBase,
          .up = yBase,
          .color = color,
          .scale = actualTextScale,
          .alignment = float2(align.x, 0.0f),
      });
}
...
void ShapeRenderer::end(DrawQueuePtr queue) {
  ...
  textMeshTexturePairs.clear();
  textMesh.finalizeMeshes(textMeshTexturePairs, WindingOrder::CW);
  ...
  for (auto &pair : textMeshTexturePairs) {
    auto drawable = std::make_shared<MeshDrawable>(pair.mesh);
    drawable->features.push_back(blendFeature);
    drawable->parameters.set("baseColorTexture", pair.texture);
#if SH_GFX_CONTEXT_DATA_LABELS
    pair.mesh->setLabel("gizmo_text");
#endif
    queue->add(drawable);
  }
...
void GizmoRenderer::addTextBillboard(float3 origin, std::string_view text, float4 color, float size, float2 align) {
  auto params = getBillboard(origin);
  shapeRenderer.addText(origin, params.x, params.y, size, text, color, align);
}
