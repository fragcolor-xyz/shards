...
// Thread-local context for Table composition
thread_local! {
  ...
  fn set_header(header: ShardsVar, settings: Option<ColumnSettings>) -> Result<(), &'static str> {
    TABLE_CONTEXT.with(|ctx| {
      ...
      if !ctx.in_table_compose {
        return Err("UI.Header can only be used within UI.Table columns");
      }
...
#[derive(shards::shard)]
#[shard_info("UI.Table", "Table layout.")]
pub struct Table {
  #[shard_required]
    ...
    let callback_type = if self.remap_key_seq {
      unsafe {
        if data.inputType.details.seqTypes.len != 1 {
          return Err("Table requires a sequence of one type");
        }
...
#[derive(shards::shard)]
#[shard_info("UI.TableHeader", "Defines a header for a Table column.")]
pub struct Header {
