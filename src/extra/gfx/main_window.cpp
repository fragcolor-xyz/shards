...
MainWindowGlobals &Base::getMainWindowGlobals() {
  MainWindowGlobals *globalsPtr = reinterpret_cast<MainWindowGlobals *>(
      _mainWindowGlobalsVar->payload.objectValue);
  if (!globalsPtr) {
...
struct MainWindow : public Base {
  ...
  CBTypeInfo compose(CBInstanceData &data) {
    ...
    shared.push_back(ExposedInfo::ProtectedVariable(
        Base::mainWindowGlobalsVarName, CBCCSTR("The graphics context"),
        MainWindowGlobals::Type));
    data.shared = shared;
  ...
  void warmup(CBContext *context) {
    ...
    _mainWindowGlobalsVar =
        referenceVariable(context, Base::mainWindowGlobalsVarName);
    ...
    if (_mainWindowGlobalsVar) {
      if (_mainWindowGlobalsVar->refcount > 1) {
        ...
        CBLOG_ERROR("MainWindow: Found {} dangling reference(s) to GFX.Context",
                    _mainWindowGlobalsVar->refcount - 1);
#endif
      }
