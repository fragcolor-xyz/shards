...
inline MeshVertexAttribute getAttributeVertexFormat(const CBVar &attribute) {
	...
	case CBType::Color:
		...
		break;
	default:
		throw formatException("Unsupported vertex attribute type {} {}", magic_enum::enum_name(attribute.valueType), uint8_t(attribute.valueType));
	}
}
...
template <typename Appender> inline void packIntoVertexBuffer(Appender &output, const CBVar &var) {
	...
	case CBType::Float4:
		...
		break;
	case CBType::Int4: {
		if (var.payload.int4Value[0] < 0 || var.payload.int4Value[0] > 255 || var.payload.int4Value[1] < 0 || var.payload.int4Value[1] > 255 ||
			var.payload.int4Value[2] < 0 || var.payload.int4Value[2] > 255 || var.payload.int4Value[3] < 0 || var.payload.int4Value[3] > 255) {
