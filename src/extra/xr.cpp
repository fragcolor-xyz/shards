...
struct Consumer {
	static inline ExposedInfo requiredInfo = ExposedInfo(ExposedInfo::Variable("XR.Context", CBCCSTR("The XR Context."), Context::ObjType));
	...
	CBTypeInfo compose(const CBInstanceData &data) {
		if (data.onWorkerThread) {
			throw ComposeError(
				"XR Blocks cannot be used on a worker thread (e.g. "
				...
				if (navigator.as<bool>()) {
					...
					if (xr.as<bool>()) {
						const auto supported =
							emscripten_wait<emscripten::val>(context, xr.call<emscripten::val>("isSessionSupported", emscripten::val("immersive-vr")));
				...
				if (xrSupported) {
					...
					if (!dialog.as<bool>()) {
						throw ActivationError(
							"Failed to find webxr permissions call "
				...
				if (isHand) {
					...
					if (pose.as<bool>()) {
						const auto transform = pose["transform"];
						{
							...
							for (int j = 0; j < 4; j++) {
								_hands[(int)hand].transform[j] =
									Var(mat[(j * 4) + 0].as<float>(), mat[(j * 4) + 1].as<float>(), mat[(j * 4) + 2].as<float>(), mat[(j * 4) + 3].as<float>());
							}
						}
						{
							...
							for (int j = 0; j < 4; j++) {
								_hands[(int)hand].inverseTransform[j] =
									Var(mat[(j * 4) + 0].as<float>(), mat[(j * 4) + 1].as<float>(), mat[(j * 4) + 2].as<float>(), mat[(j * 4) + 3].as<float>());
							}
						}
