...
struct SetTableValue {
  ...
  CBTypeInfo inferTypes(CBTypeInfo inputType,
    ...
    tableExposedInfo = ExposedInfo(ExposedInfo::Variable(
        name.c_str(), "The exposed table.", CBTypeInfo(tableTypeInfo)));
    return inputType;
  }
...
struct GetTableValue {
  ...
  CBExposedTypesInfo consumedVariables() {
    if (!tableExposedInfo) {
      ...
      CBExposedTypeInfo nameInfo = {name.c_str(), "The exposed table.",
                                    atableType};
    ...
    for (auto i = 0; i < stbds_arrlen(consumableVariables); i++) {
      if (consumableVariables[i].name == name &&
        ...
        return consumableVariables[i]
            .exposedType.tableTypes[0]; // TODO taking only the first for now
      }
    }
    assert(false);
    return CBTypeInfo();
