...
m3ApiRawFunction(m3_wasi_unstable_args_sizes_get) {
  ...
  SHLOG_TRACE("WASI args_sizes_get");
...
m3ApiRawFunction(m3_wasi_unstable_environ_sizes_get) {
  ...
  SHLOG_TRACE("WASI environ_sizes_get");
...
m3ApiRawFunction(m3_wasi_unstable_fd_prestat_dir_name) {
  ...
  SHLOG_TRACE("WASI fd_prestat_dir_name");
...
m3ApiRawFunction(m3_wasi_unstable_fd_prestat_get) {
  ...
  SHLOG_TRACE("WASI fd_prestat_get");
...
m3ApiRawFunction(m3_wasi_unstable_fd_fdstat_get) {
  ...
  SHLOG_TRACE("WASI fd_fdstat_get");
...
m3ApiRawFunction(m3_wasi_unstable_fd_filestat_get) {
  ...
  SHLOG_TRACE("WASI m3_wasi_unstable_fd_filestat_get");
...
m3ApiRawFunction(m3_wasi_unstable_fd_fdstat_set_flags) {
  ...
  SHLOG_TRACE("WASI m3_wasi_unstable_fd_fdstat_set_flags");
...
m3ApiRawFunction(m3_wasi_unstable_fd_seek) {
  ...
  SHLOG_TRACE("WASI m3_wasi_unstable_fd_seek");
...
m3ApiRawFunction(m3_wasi_unstable_path_open) {
  ...
  SHLOG_TRACE("WASI m3_wasi_unstable_path_open");
  ...
  SHLOG_TRACE("WASI path_open: {}", std::string_view(host_path, path_len));
...
m3ApiRawFunction(m3_wasi_unstable_fd_read) {
  ...
  SHLOG_TRACE("WASI fd_read, fd: {}", fd);
...
m3ApiRawFunction(m3_wasi_unstable_fd_write) {
  ...
  SHLOG_TRACE("WASI fd_write, fd: {}", fd);
...
m3ApiRawFunction(m3_wasi_unstable_fd_close) {
  ...
  SHLOG_TRACE("WASI m3_wasi_unstable_fd_close");
...
m3ApiRawFunction(m3_wasi_unstable_fd_datasync) {
  ...
  SHLOG_TRACE("WASI m3_wasi_unstable_fd_datasync");
...
m3ApiRawFunction(m3_wasi_unstable_random_get) {
  ...
  SHLOG_TRACE("WASI m3_wasi_unstable_random_get");
...
m3ApiRawFunction(m3_wasi_unstable_clock_res_get) {
  ...
  SHLOG_TRACE("WASI m3_wasi_unstable_clock_res_get");
...
m3ApiRawFunction(m3_wasi_unstable_clock_time_get) {
  ...
  SHLOG_TRACE("WASI m3_wasi_unstable_clock_time_get");
...
m3ApiRawFunction(m3_wasi_unstable_proc_exit) {
  ...
  SHLOG_TRACE("WASI m3_wasi_unstable_proc_exit");
...
struct Run {
  static constexpr SHString wasmExt = ".wasm";
  static constexpr SHStrings wasmExts = {(const char **)&wasmExt, 1, 0};
  static constexpr SHTypeInfo wasmFileType{SHType::Path, {.path = {wasmExts, true, true, true}}};
  ...
  static inline Parameters params{
      {"Module", SHCCSTR("The wasm module to run."), {WasmFilePath, CoreInfo::StringType}},
      {"Arguments",
       SHCCSTR("The arguments to pass to the module entrypoint function."),
       {CoreInfo::NoneType, CoreInfo::StringSeqType, CoreInfo::StringVarSeqType}},
      {"EntryPoint", SHCCSTR("The entry point function to call when activating."), {CoreInfo::StringType}},
      {"StackSize", SHCCSTR("The stack size in kilobytes to use."), {CoreInfo::IntType}},
      {"ResetRuntime",
       SHCCSTR("If the runtime should be reset every activation, altho slow "
               "this might be useful if certain modules fail to execute "
               "properly or leak on multiple activations."),
       {CoreInfo::BoolType}},
      {"CallConstructors",
       SHCCSTR("Use if it might be necessary to force a call to "
        ...
        [&]() {
          ...
          if (_entryPoint != "_start") {
            ...
            if (argsVar.valueType == Seq) {
              for (auto &arg : argsVar) {
                ...
                SHLOG_TRACE("WASM entrypoint argument: {}", _argsArray.back());
              }
            }
            ...
            if (argsVar.valueType == Seq) {
              for (auto &arg : argsVar) {
                ...
                SHLOG_TRACE("WASM WASI argument: {}", _data.args.back());
              }
            }
          ...
          if (result == m3Err_trapExit) {
            if (_data.exit_code != 0) {
              _serr.done();
              _sout.done();
              SHLOG_INFO(_sout.str());
              SHLOG_ERROR(_serr.str());
            ...
            SHLOG_INFO(_sout.str());
            SHLOG_ERROR(_serr.str());
            SHLOG_ERROR(_runtime->error_message);
...
void registerShards() { REGISTER_SHARD("Wasm.Run", Wasm::Run); }
