...
#define RUNTIME_BLOCK(_namespace_, _name_)                                                                                \
  ...
  __cdecl CBlock *createBlock##_name_() {                                                                                 \
    CBlock *result = reinterpret_cast<CBlock *>(new (std::align_val_t{16}) _name_##Runtime());                            \
    result->name = static_cast<CBNameProc>([](CBlock *block) { return #_namespace_ "." #_name_; });                       \
    result->hash = static_cast<CBHashProc>([](CBlock *block) {                                                            \
      return ::chainblocks::constant<::chainblocks::crc32(#_namespace_ "." #_name_ CHAINBLOCKS_CURRENT_ABI_STR)>::value;  \
    });                                                                                                                   \
    result->help = static_cast<CBHelpProc>([](CBlock *block) { return CBOptionalString(); });                             \
    ...
    result->requiredVariables = static_cast<CBRequiredVariablesProc>([](CBlock *block) { return CBExposedTypesInfo(); }); \
    result->parameters = static_cast<CBParametersProc>([](CBlock *block) { return CBParametersInfo(); });                 \
    result->setParam = static_cast<CBSetParamProc>([](CBlock *block, int index, const CBVar *value) {});                  \
    ...
    result->inputHelp = static_cast<CBHelpProc>([](CBlock *block) { return CBOptionalString(); });                        \
    result->outputHelp = static_cast<CBHelpProc>([](CBlock *block) { return CBOptionalString(); });                       \
    result->properties = static_cast<CBPropertiesProc>([](CBlock *block) -> const CBTable * { return nullptr; });         \
...
#define RUNTIME_CORE_BLOCK(_name_)                                                                                        \
  ...
  __cdecl CBlock *createBlock##_name_() {                                                                                 \
    ...
    result->hash = static_cast<CBHashProc>([](CBlock *block) {                                                            \
      return ::chainblocks::constant<::chainblocks::crc32(#_name_ CHAINBLOCKS_CURRENT_ABI_STR)>::value;                   \
    });                                                                                                                   \
    result->help = static_cast<CBHelpProc>([](CBlock *block) { return CBOptionalString(); });                             \
    result->setup = static_cast<CBSetupProc>([](CBlock *block) {});                                                       \
    result->destroy = static_cast<CBDestroyProc>([](CBlock *block) {                                                      \
...
#define RUNTIME_BLOCK_inputTypes(_name_)              \
  result->inputTypes = static_cast<CBInputTypesProc>( \
      [](CBlock *block) { return reinterpret_cast<_name_##Runtime *>(block)->core.inputTypes(); });
#define RUNTIME_BLOCK_outputTypes(_name_)               \
  result->outputTypes = static_cast<CBOutputTypesProc>( \
      [](CBlock *block) { return reinterpret_cast<_name_##Runtime *>(block)->core.outputTypes(); });

#define RUNTIME_BLOCK_exposedVariables(_name_)                    \
  result->exposedVariables = static_cast<CBExposedVariablesProc>( \
      [](CBlock *block) { return reinterpret_cast<_name_##Runtime *>(block)->core.exposedVariables(); });
#define RUNTIME_BLOCK_requiredVariables(_name_)                     \
  result->requiredVariables = static_cast<CBRequiredVariablesProc>( \
      [](CBlock *block) { return reinterpret_cast<_name_##Runtime *>(block)->core.requiredVariables(); });

#define RUNTIME_BLOCK_parameters(_name_)              \
  result->parameters = static_cast<CBParametersProc>( \
      [](CBlock *block) { return reinterpret_cast<_name_##Runtime *>(block)->core.parameters(); });
...
#define RUNTIME_BLOCK_getParam(_name_)            \
  result->getParam = static_cast<CBGetParamProc>( \
      [](CBlock *block, int index) { return reinterpret_cast<_name_##Runtime *>(block)->core.getParam(index); });

#define RUNTIME_BLOCK_compose(_name_)           \
  result->compose = static_cast<CBComposeProc>( \
      [](CBlock *block, CBInstanceData data) { return reinterpret_cast<_name_##Runtime *>(block)->core.compose(data); });

#define RUNTIME_BLOCK_warmup(_name_)          \
  result->warmup = static_cast<CBWarmupProc>( \
      [](CBlock *block, CBContext *ctx) { reinterpret_cast<_name_##Runtime *>(block)->core.warmup(ctx); });

#define RUNTIME_BLOCK_activate(_name_)                                                                       \
  result->activate = static_cast<CBActivateProc>([](CBlock *block, CBContext *context, const CBVar *input) { \
    return reinterpret_cast<_name_##Runtime *>(block)->core.activate(context, *input);                       \
  });

#define RUNTIME_BLOCK_cleanup(_name_) \
  result->cleanup = static_cast<CBCleanupProc>([](CBlock *block) { reinterpret_cast<_name_##Runtime *>(block)->core.cleanup(); });

#define RUNTIME_BLOCK_mutate(_name_)          \
  result->mutate = static_cast<CBMutateProc>( \
      [](CBlock *block, CBTable options) { reinterpret_cast<_name_##Runtime *>(block)->core.mutate(options); });
