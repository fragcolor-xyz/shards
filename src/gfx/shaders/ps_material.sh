...
void materialSetMetallicRoughness(inout MaterialInfo mi, float u_MetallicFactor, float u_RoughnessFactor) {
	...
	mi.diffuseColor = mix(mi.DEFAULT_COLOR_FIELD.rgb * (vec3_splat(1.0) - mi.specularColor0), vec3_splat(0), mi.metallic);
...
void materialSetSpecularColor(inout MaterialInfo mi, vec3 color, float weight) {
	...
	mi.specularColor0 = mix(dielectricSpecularF0, mi.DEFAULT_COLOR_FIELD, mi.metallic);
	mi.specularWeight = weight;
	mi.diffuseColor = mix(mi.DEFAULT_COLOR_FIELD.rgb * (1.0 - max3(dielectricSpecularF0)), vec3_splat(0), mi.metallic);
}
#endif
