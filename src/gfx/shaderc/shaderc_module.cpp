...
bool compileShader(const char *_varying, const char *_comment, char *_shader,
                   uint32_t _shaderLen, bgfx::Options &_options,
                   bx::WriterI *_writer);
extern bool g_verbose;
} // namespace bgfx

struct ShaderCompiler : public gfx::IShaderCompiler {
  virtual bool compile(gfx::IShaderCompileOptions &options,
                       gfx::IShaderCompileOutput &output, const char *varying,
                       const char *code, size_t codeLength) {
...
#define CONVERT_ARRAY(_res, _get, _size)                                       \
  for (size_t i = 0; i < (_size); i++) {                                       \
    ...
    CONVERT_ARRAY(bgfxOptions.includeDirs, options.getIncludeDir,
                  options.getNumIncludeDirs())
    CONVERT_ARRAY(bgfxOptions.defines, options.getDefine,
                  options.getNumDefines())
    CONVERT_ARRAY(bgfxOptions.dependencies, options.getDependency,
                  options.getNumDependencies())
    ...
    std::string tempFilePath = []() {
      ...
      bx::stringPrintf(result, ".shaderSource%llu.txt", counter);

      return result;
    }();

    bx::FileWriter tempFileWriter;
    if (!bx::open(&tempFileWriter, tempFilePath.c_str())) {
      return false;
    }
    ...
    bgfxOptions.inputFilePath = tempFilePath;

    bgfx::g_verbose = podOptions.verbose;
    bool shaderCompiled = bgfx::compileShader(varying, "", codeCopy, codeLength,
                                              bgfxOptions, &writer);

    // remove temp file
    bx::remove(tempFilePath.c_str());
...
extern "C" {
gfx::IShaderCompiler *SHADERC_API GFX_SHADERC_MODULE_ENTRY() {
