...
using Color = byte4;
using Colorf = float4;
inline Colorf colorToFloat(const Color &color) {
	return Colorf(color) / Colorf(255.0f);
}
inline Color colorFromFloat(const Colorf &color) {
	return Color(color * Colorf(255.0f));
}
