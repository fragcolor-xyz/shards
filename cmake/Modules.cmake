...
function(shards_generate_rust_union TARGET_NAME)
  ...
  file(WRITE ${LIB_RS} "// This file is generated by CMake\n")
  ...
  foreach(RUST_TARGET ${RUST_TARGETS})
    ...
    message(STATUS "${TARGET_NAME}: Adding rust target ${RUST_TARGET} (path: ${RUST_PROJECT_PATH}, name: ${RUST_NAME})")

    file(RELATIVE_PATH RELATIVE_RUST_PROJECT_PATH ${GEN_RUST_PATH} ${RUST_PROJECT_PATH})
    string(APPEND CARGO_CRATE_DEPS "${RUST_NAME} = { path = \"${RELATIVE_RUST_PROJECT_PATH}\" }\n")

    string(REPLACE "-" "_" RUST_NAME_ID ${RUST_NAME})
    file(APPEND ${LIB_RS} "pub use ${RUST_NAME_ID};\n")

    get_property(LINK_LIBS TARGET ${RUST_TARGET} PROPERTY INTERFACE_LINK_LIBRARIES)
    list(APPEND COMBINED_INTERFACE_LINK_LIBRARIES ${LINK_LIBS})
  endforeach()
  ...
  if(COMBINED_INTERFACE_LINK_LIBRARIES)
    target_link_libraries(${TARGET_NAME}-rust INTERFACE ${COMBINED_INTERFACE_LINK_LIBRARIES})
    message(STATUS "${TARGET_NAME}: Linking against ${COMBINED_INTERFACE_LINK_LIBRARIES}")
  endif()
endfunction()
